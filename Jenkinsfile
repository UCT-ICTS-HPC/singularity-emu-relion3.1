...
node('gpu') {
    ...
    stage('Build') {
      ...
      sh "sudo $SINGULARITY_BIN build --notest $CONTAINER_DIR/$CONTAINER_NAME-$APP_VER-$BUILD_NUMBER $CONTAINER_DEF"
    }
    ...
    stage('Running Tests') {
      ...
      sh "$SINGULARITY_BIN test --nv $CONTAINER_DIR/$APP_VER-$CONTAINER_NAME-$BUILD_NUMBER "
    }

    stage('Deliver HPC software to repository') {
      // Make application available to HPC users 
      sh "cp $CONTAINER_DIR/$APP_VER-$CONTAINER_NAME-$BUILD_NUMBER $SW_LOCATION"
      dir(SW_LOCATION) {
         sh "ln -sf $SW_LOCATION/$APP_VER-$CONTAINER_NAME-$BUILD_NUMBER $CONTAINER_NAME-$APP_VER.$CONTAINER_FMT"
      }
